...
- **Audio source IDs**: `AudioSource` enum at `backend/core/models/audio_state.py` (`none`, `spotify`, `bluetooth`, `radio`, `podcast`, `airplay`, `mac`, `cd`, `dlna`, `qobuz`, `tidal`, `music_library`). The Swift side uses bare strings — they must match the enum values byte-for-byte. Watch out for `music_library`: the backend also uses a hyphenated `music-library` for its route prefix, view keys and icon filenames, but **never** as the source id.
- **The audio state wire**: the frozen section « Développeurs : le fil » of the Milō audio-states document (24/09/2026, phase 5.0) is the contract for `/api/audio/state` and the `source/state` event. It is decoded by `MiloAudioState.swift`, which is **byte-identical to `Milo-iOS/Shared/MiloAudioState.swift`** — both apps read the wire through the same file, so a change goes into both repos (`diff` them; the header says how). Nothing else here parses that JSON. `service` (`stopped`/`starting`/`running`/`failed`) and `session.phase` (`loading`/`playing`/`paused`/`connected`) are frozen enums: an unknown value fails the decode on purpose, and the event is dropped with a log. There is no compatibility layer with the old `source_state`/`transitioning`/`metadata` wire — this app and the backend ship together.
- **Derived tests live in one place**: `MiloAudioState.isSourceSettled` (`service == .running && !switching`, the gate for every sub-level: radio stations, music-library search) and `isSourceStarting` (`service == .starting || switching`, what the source spinner follows) are in the Mac-only extension in `MiloAPIService.swift`. The now-playing row shows `MiloAudioState.shown` (the session if titled, else the resume point), and its buttons follow `controls` — never a hardcoded per-source list: the backend lists exactly the commands it accepts right now.
...
`MiloAudioState` is a `Sendable` value type (a `Decodable` struct of structs and enums), which matters because it is decoded off-main and handed to the main actor. `MusicLibrarySong.raw` is the one untyped dict that crosses that boundary: it is `[String: any Sendable]`, built by `JSONSendable.dictionary(_:)`. **Do not "simplify" that to `json as? [String: any Sendable]`** — `Sendable` is a marker protocol with no runtime representation, so that cast *always succeeds without checking anything*. It would be an `@unchecked` in disguise. The explicit rebuild keeps `NSNumber` values as-is, so `as? Int` / `as? String` reads behave identically.
...
- `manualLoadingProtection` — a **grace window** (`manualLoadingGraceDuration`, 2 s), *not* a minimum display time. It only bridges the click→`switching` race, so a stale pre-transition broadcast can't clear a spinner before the backend takes over. The entry is dropped the moment a `switching`/`service == starting` state for that source arrives; after that the spinner clears as soon as the backend reports the transition done, matching the web frontend's timing.
...
- A source click shows its spinner immediately, so the clear path has to be airtight: `syncLoadingStatesWithBackend` schedules a deferred re-check at the end of the grace window (`scheduleGraceWindowSourceLoadingClear`), so a fast transition that settles with no further broadcast still clears instead of hanging until the 15 s safety timer.
- Source transitions resolve by observing `isSourceStarting` (`service == .starting || switching`); feature toggles resolve by matching incoming state against `expectedFunctionalityStates`. Multiroom needs one more condition: the backend pre-sets `multiroom_enabled` *before* doing the real routing work, so intermediate states already carry the new value. `switching` stays true for the whole multiroom switch (volume sync included), so the spinner resolves on the first state with the expected value **and** `switching == false`. A refusal still arrives as `routing/multiroom_error`.
