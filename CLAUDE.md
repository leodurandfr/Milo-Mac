...
- **Source operational states**: `SourceState` enum in the same file (`starting`, `ready`, `active`, `error`). `MiloState.sourceState` stores the raw string; compare lowercased — via `MiloState.isSourceSettled` for the "engine up, not in flight" test that gates every sub-level (radio stations, music-library search). `ready` was called `waiting` until the backend renamed it; the helper accepts both, because a Pi running an older backend still sends `waiting`. That rename is what silently removed both chevrons once — when a state string stops matching, the symptom is a feature that quietly disappears, not an error.
