...
- **Product vs. project name**: the Xcode target's `PRODUCT_NAME` is `Milo` (bundle id `leodurand.Milo-Mac`), but the repo/folder is `Milo Mac` and the app is branded **Milō** (macron). The string `Milō` appears in user-facing text; don't strip the macron. Two schemes exist: `Milo Mac.xcscheme` (primary) and `Sonoak Menu.xcscheme` (legacy/alternate — ask before touching).
