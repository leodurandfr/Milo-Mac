...
- **`milo.local` discovery**: `MiloConnectionManager` uses a state machine (`ConnectionPhase`: idle → discovering → testingAPI → connecting → connected) driven by mDNS (`NetServiceBrowser` on `_http._tcp`), then 20 rapid API readiness checks (`fetchState()` → `/api/audio/state`, not a dedicated health route), then the WebSocket. When multiple IPv4s resolve, it picks the lowest-latency one via a TCP probe. If you add connection logic, plug into the phase machine — don't bypass it.
- **Two transports, complementary**: HTTP (`MiloAPIService`) for commands and initial state; WebSocket (`WebSocketService`, port 8000, path `/ws`) for push updates. **`volume/volume_changed` events don't carry limits** — `MenuBarController.didReceiveVolumeUpdate` preserves them from the cached `currentVolume`. Don't regress that: re-introducing 0-valued limits bricks the slider.
- **Settings (volume limits + dock apps) come from `/api/settings/bulk`, cached**: fetched once per connection (`MiloAPIService.fetchBulkSettings`) and cached, so the volume HUD never re-pulls the bulk payload. They update live via WebSocket `settings/volume_limits_changed` and `settings/dock_apps_changed` (→ `MenuBarController.didReceiveVolumeLimitsUpdate` / `didReceiveDockAppsUpdate`), like the Milō frontend's settingsStore. Milo-Mac no longer calls the old per-category routes (`/api/settings/{volume-limits,volume-steps,dock-apps}`) — don't reintroduce them. The keyboard-shortcut volume step is **local** (`GlobalHotkeyManager.volumeDeltaDb`, 1–6 dB in Settings), not the backend's `step_mobile_db`; `VolumeStatus` has no step field.
...
- **Loading state is orchestrated, not reactive**: `MenuBarController` has `loadingStates`, `manualLoadingProtection` (a **grace window**, `manualLoadingGraceDuration` 2 s — not a fixed display floor: it only bridges the click→`transition_start` race so a stale pre-transition broadcast can't clear the spinner before the backend takes over; the entry is cleared the moment a `transitioning`/`starting` state for that source arrives, after which the spinner resolves the instant the backend reports the transition done, matching the web frontend's timing), and `minimumFunctionalityLoadingDuration` (1.2 s, a genuine display floor for feature toggles). A source-click spinner shows immediately (`createAudioSourcesSection` ORs `loadingStates` into the transition check), so its clear path must stay airtight: `syncLoadingStatesWithBackend` schedules a deferred re-check at the grace-window end (`scheduleGraceWindowSourceLoadingClear`) so a fast transition that settles in `WAITING` with no further broadcast still clears instead of hanging until the 15 s safety timer. Source transitions are auto-resolved by observing `sourceState == "starting"` / `transitioning` on incoming state; feature toggles (`multiroom`, `equalizer`) are resolved by matching the new state against `expectedFunctionalityStates`. Don't add ad-hoc `isLoading` flags.
